...
class Solution:
    def __init__(self):
        ...
        self.RESPONSES_PATH = "llm_responses"
        self.REQUESTS_PATH = "llm_requests"
        self.OUTPUTS_PATH = "python_outputs"
        self.PUZZLE_FILE = "puzzle.md"

        self.THRESHOLD = 2

    def yearDayPath(self, year: int, day: int) -> Path:
        return Path(f"{year}-{day}")

    def writeInstructions(day: int, year: int) -> None:
        subprocess.run(
    ...
    def readInstructions(self, day: int) -> str:
        return Path(PUZZLE_FILE).read_text()

    def extractPart1(self, text: str) -> str:
    ...
    def extractPart2(self, text: str) -> str:
        text = re.sub(".*--- Day.*", "", text)
        text = re.sub(".*-----", "", text)
        text, _ = re.subn("Your puzzle answer was.*", "", text)
    ...
    def amendInstructions(self, text: str, part):
        ...
        text = f"{self.llm_prefix}{textwrap.indent(text, '    ')}\n\n-----\n\n{llm_instructions}"
        (Path(self.REQUESTS_PATH) / f"part-{part}.txt").write_text(text)
    ...
    def do_part(n: int, day: int, part: int) -> str | None:
        ...
        instructions = self.readInstructions(day)
        if part == 1:
            instructions = amendInstructions(
                extractPart1(instructions), part=part)
        elif part == 2:
            instructions = amendInstructions(
                extractPart2(instructions), part=part)
        else:
            raise ValueError

        try:
            llm_response = requestPythonFromLLM(instructions)
            ...
            (Path(self.RESPONSES_PATH) /
             f"part_{part}_{n}.error").write_text(str(e))
            return None
        (Path(self.RESPONSES_PATH) /
         f"part_{part}_{n}.py").write_text(llm_response)
        try:
            # Run the code, writing the output to a file
            f = Path(self.OUTPUTS_PATH) / f"part_{part}_{n}.txt"
    ...
    def request_python_from_llm(self, instructions) -> str:
        llm_command = f"{self.LLM_COMMAND_PREFIX} {instructions}"
    ...
    def parseAnswer(self, text: str) -> Iterable[str]:
        """
    ...
    def readResults(part: int) -> Counter:
        c: Counter = Counter()
        for n in range(1, 100):
            f = Path(self.OUTPUTS_PATH) / f"part_{part}_{n}.txt"
            if not f.exists():
                continue
            answer = self.parseAnswer(f.read_text())
            if answer:
                c.update(answer)

        return c

    def submitResult(self, day, year, part, answer):
        self.logger.info(f"Submitting {answer} for part {part}")
            ...
            for f in futures.as_completed(fs):
                ...
                if len(top_results) >= 1:
                    ...
                    if top - next >= threshold:
                        print(
                            f"Submitting {top_results[0][0]}. Counts were {c}")
                        out = submit_result(
                            day, year, part, c.most_common(1)[0][0])
                        print(out)
                        submitted = True
                        if stop_when_submitted:
                            print(
                                f"Now stopping on part {part}. Final results: {c}")
                            ...
                            return
                print(
                    f"No answer hit the threshold, not submitting. Current counts: {c}")
            print(
                f"Reached all attempts without success, stopping. Final results: {c}")
            return
    ...
    def run(self, day: int, year: int, part: int, n_workers: int, runs: int, stop_when_submitted: bool) -> None:
        ...
        writeInstructions(day, year)

        if part is None:
            runParallel(day, year, 1, stop_when_submitted, n_workers, runs)
            runParallel(day, year, 2, stop_when_submitted, n_workers, runs)
        else:
            if n_workers == 1:
                doPart(0, day, part)
            else:
                runParallel(day, year, part,
                            stop_when_submitted, n_workers, runs)


if __name__ == "__main__":
    solution = Solution()
    solution.logging.basicConfig(level=logging.INFO, force=True)
    solution.run()
    sys.exit(0)
